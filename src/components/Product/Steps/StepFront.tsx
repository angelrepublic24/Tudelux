...
export const StepFront = ({
  ...
  const isValidAddOnCombo = (
    newAddOn: string,
    position: string,
    currentMaterials: MaterialItemTable[]
  ): boolean => {
    ...
    const notAllowed = incompatibilities[newAddOn] || [];

    const samePositionAddOns = currentMaterials
      .filter((mat) => mat.name.includes(position))
      .map((mat) => mat.name.split(" - ")[0]);

    return !samePositionAddOns.some((existing) =>
      notAllowed.includes(existing)
    );
  };
  ...
  const handleSelection = (
    ...
    selectedButtons.forEach((btn) => {
      if (shape === "Rectangular") {
        ...
        const allCurrentMaterials = JSON.parse(JSON.stringify(materialsData));

        if (!isValidAddOnCombo(name, btn, allCurrentMaterials)) {
          toast.error(
            `❌ "${name}" cannot be placed on "${btn}" due to a conflict with an existing add-on.`
          );
          return;
        }
          ...
          .map((addon: RawAddOn, i: number) => {
            ...
            return (
              <ChooseDesignGrid
                ...
                validateCombo={(btn: string) =>
                  isValidAddOnCombo(addOnValue.name, btn, materialsData)
                }
